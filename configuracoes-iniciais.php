...
<!DOCTYPE html>
	<html xmlns="http://www.w3.org/1999/xhtml">
		<head>
			...
			<title>Sistema de gest&atilde;o MPress &rsaquo; Arquivo de Configuração da Instalação</title>
		...
		<body>
			...
			<?php if($_GET['passo']==""){?>
				<div id='passo-1'>
					<p Style='margin-top:25px'>Bem-vindo ao Sistema de gest&atilde;o MPress. Antes de come&ccedil;ar, precisamos de algumas informa&ccedil;&otilde;es sobre o banco de dados e hospedagem.</p>
					<ol>
				...
				<div id='passo-2'>
					<form id='form-inicio' method="post" action="configuracoes-iniciais.php?passo=3">
						...
						<table class="form-table" width='100%' border='0'>
							...
							<tr>
								<th scope="row"><label for="expira">Data Expira</label></th>
								<td><input name="expira" id="expira" type="text" /></td>
							</tr>
							<tr>
								<th scope="row"><label for="objeto">Nomenclatura</label></th>
								<td><input name="objeto" id="objeto" type="text" /></td>
							</tr>
						</table>
			...
			<?php
					...
					$config['title']	= utf8_decode($_POST['prefix']);
					$config['descr']	= utf8_decode($_POST['descricao']);
					$config['expira']	= $_POST['expira'];
					$config['objeto']	= utf8_decode($_POST['objeto']);
					...
					$conteudoArquivo = str_replace("[data-expira]", $config['expira'] ,$conteudoArquivo);
					$conteudoArquivo = str_replace("[objeto]", $config['objeto'] ,$conteudoArquivo);
