...
<?php
	if ($_POST){
		...
		$sql = "select count(cw.Workflow_ID) as Cont, coalesce(ce.UF,'N/A') as UF, upper(ce.Cidade) as Cidade
			from chamados_workflows cw
				inner join cadastros_enderecos ce on cw.Solicitante_ID = ce.Cadastro_ID
							and ce.Tipo_Endereco_ID = 26 and ce.Cadastro_Endereco_ID = (select ceaux.Cadastro_Endereco_ID from cadastros_enderecos ceaux where ceaux.Cadastro_ID = ce.Cadastro_ID limit 1  )
		...
		$sql = "select count(cw.Workflow_ID) as Cont, coalesce(ce.UF,'N/A') as UF
			from chamados_workflows cw
				inner join cadastros_enderecos ce on cw.Solicitante_ID = ce.Cadastro_ID
							and ce.Tipo_Endereco_ID = 26 and ce.Cadastro_Endereco_ID = (select ceaux.Cadastro_Endereco_ID from cadastros_enderecos ceaux where ceaux.Cadastro_ID = ce.Cadastro_ID limit 1  )
				where cw.Workflow_ID is not null
