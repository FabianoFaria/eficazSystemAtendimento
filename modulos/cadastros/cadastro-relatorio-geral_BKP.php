<?php
				...
				<div class='titulo-container'>
					...
					<div class='conteudo-interno'>
						...
						<div class='titulo-secundario oito-colunas'>
							...
							<p><select name='localiza-areas-atuacoes' id='localiza-areas-atuacoes' style='width:50%;'>".optionValueGrupo(34, $areasAtuacoes,'&nbsp;')."</select></p>
						</div>
						<div class='titulo-secundario oito-colunas'>
	...
	if($_POST){
		...
		while($row = mpress_fetch_array($resultado)){
			if ($row[Foto]!="")
				$arrayCadastros[$row[Cadastro_ID]]['Foto'] = "<img src='$caminhoSistema/uploads/".$row[Foto]."' width='50px' id='imagem-foto'>";
			else
				$arrayCadastros[$row[Cadastro_ID]]['Foto'] = "<img src='$caminhoSistema/images/geral/imagem-usuario.jpg' width='50px' id='imagem-foto'>";
		...
		if ($_POST['separar-por']=="cc"){
			...
			foreach($arrayCadastros as $cadastro_ID => $dadosCadastro){
				if($dadosCadastro['CentroCustoID']!= $ccIDAnt){
					$j++;
					$i=0;

					if ($dadosCadastro['Situacao']=="1") $classe = "link"; else $classe = "lixeira";
					if ($dadosCadastro['CentroCusto']!="") $centroCusto[$j] = $dadosCadastro['CentroCusto']; else $centroCusto[$j] = "N/A";
					if ($dadosCadastro['Responsavel']!="") $responsavel[$j] = $dadosCadastro['Responsavel']; else $responsavel[$j] = "N/A";

					$largura = "100.2%";
					$colunas = "10";
					$dados[$j][colunas][tamanho][1] = "width='30px' height='40px'";
					$dados[$j][colunas][tamanho][2] = "width='6%'";
					$dados[$j][colunas][tamanho][3] = "width='10%'";
					$dados[$j][colunas][tamanho][4] = "width='100px'";
					$dados[$j][colunas][tamanho][5] = "";
					$dados[$j][colunas][tamanho][6] = "";
					$dados[$j][colunas][tamanho][7] = "";
					$dados[$j][colunas][tamanho][8] = "";
					$dados[$j][colunas][tamanho][9] = "";

					$dados[$j][colunas][titulo][1] 	= "Foto";
					$dados[$j][colunas][titulo][2] 	= "ID";
					$dados[$j][colunas][titulo][3] 	= "C&oacute;digo";
					$dados[$j][colunas][titulo][4] 	= "Tipo";
					$dados[$j][colunas][titulo][5] 	= "Nome";
					$dados[$j][colunas][titulo][6] 	= "Cpf / Cnpj";
					$dados[$j][colunas][titulo][7] 	= "Email";
					$dados[$j][colunas][titulo][8] 	= "Telefone";
					$dados[$j][colunas][titulo][9] 	= "Endereço";
					$dados[$j][colunas][titulo][10] = "Regional";


					$dados[$j][colunas][ordena][2] = "Cadastro_ID";
					$dados[$j][colunas][ordena][3] = "cast(codigo AS SIGNED)";
					$dados[$j][colunas][ordena][4] = "Tipo_Pessoa";
					$dados[$j][colunas][ordena][5] = "Nome";
					$dados[$j][colunas][ordena][6] = "Cpf_Cnpj";
					$dados[$j][colunas][ordena][7] = "Email";
				}

				$i++;
				$dados[$j][colunas][conteudo][$i][1] = "<span class='$classe cadastro-localiza' cadastro-id='".$cadastro_ID."'><p width='30px' height='40px' class='cadastro-localiza' cadastro-id='".$row[Cadastro_ID]."'>".$dadosCadastro['Foto']."</p></span>";
				$dados[$j][colunas][conteudo][$i][2] = "<span class='$classe cadastro-localiza' cadastro-id='".$cadastro_ID."'><p Style='margin:2px 2px 0 2px;float:left;'>".$cadastro_ID."</p></span>";
				$dados[$j][colunas][conteudo][$i][2] = "<span class='$classe cadastro-localiza' cadastro-id='".$cadastro_ID."'><p Style='margin:2px 2px 0 2px;float:left;'>".$cadastro_ID."</p></span>";
				$dados[$j][colunas][conteudo][$i][3] = "<span class='$classe cadastro-localiza' cadastro-id='".$cadastro_ID."'><p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Codigo']."</p></span>";
				$dados[$j][colunas][conteudo][$i][4] = "<p class='$classe' Style='margin:2px 2px 0 2px;float:left; white-space:nowrap;'>".$dadosCadastro['Tipo']."</p>";
				$dados[$j][colunas][conteudo][$i][5] = "<span class='$classe cadastro-localiza' cadastro-id='".$cadastro_ID."'><p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Nome']."</p></span>";
				$dados[$j][colunas][conteudo][$i][6] = "<p Style='margin:2px 2px 0 2px;float:left; white-space:nowrap;'>".$dadosCadastro['CpfCnpj']."</p>";
				$dados[$j][colunas][conteudo][$i][7] = "<p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Email']."</p>";
				$dados[$j][colunas][conteudo][$i][8] = "<p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Telefones']."</p>";
				$dados[$j][colunas][conteudo][$i][9] = "<p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Enderecos']."</p>";
				$dados[$j][colunas][conteudo][$i][10] = "<p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Regional']."</p>";

				$ccIDAnt = $dadosCadastro['CentroCustoID'];
			}
		}
		else{
			$i=0;
			...
			$largura = "100.2%";
			$colunas = "10";
			$dados[colunas][tamanho][1] = "width='30px' height='40px'";
			...
			foreach($arrayCadastros as $cadastro_ID => $dadosCadastro){
				...
				$dados[colunas][conteudo][$i][2] = "<span class='$classe cadastro-localiza' cadastro-id='".$cadastro_ID."'><p Style='margin:2px 2px 0 2px;float:left;'>".$cadastro_ID."</p></span>";
				$dados[colunas][conteudo][$i][3] = "<span class='$classe cadastro-localiza' cadastro-id='".$cadastro_ID."'><p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Codigo']."</p></span>";
				$dados[colunas][conteudo][$i][4] = "<p class='$classe' Style='margin:2px 2px 0 2px;float:left; white-space:nowrap;'>".$dadosCadastro['Tipo']."</p>";
				$dados[colunas][conteudo][$i][5] = "<span class='$classe cadastro-localiza' cadastro-id='".$cadastro_ID."'><p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Nome']."</p></span>";
				$dados[colunas][conteudo][$i][6] = "<p Style='margin:2px 2px 0 2px;float:left; white-space:nowrap;'>".$dadosCadastro['CpfCnpj']."</p>";
				$dados[colunas][conteudo][$i][7] = "<p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Email']."</p>";
				$dados[colunas][conteudo][$i][8] = "<p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Telefones']."</p>";
				$dados[colunas][conteudo][$i][9] = "<p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Enderecos']."</p>";
				$dados[colunas][conteudo][$i][10] = "<p Style='margin:2px 2px 0 2px;float:left;'>".$dadosCadastro['Regional']."</p>";
			}
		}

		if($i==0)
			echo "<p Style='margin:2px 2px 0 2px;color:red; text-align:center'>Nenhum cadastro localizado</p>";
			...
			$html = "	<div class='conteudo-interno-retorno' id='conteudo-interno-retorno'>
							<table width='100%' Style='margin-top:0px;border:1px solid silver;margin-bottom:2px;' cellpadding='2' cellspacing='2' align='center'>
								<tr class='esconde'>
									<td background='$caminhoSistema/images/documentos/cabecalho.jpg' height='70px' style='font-family:arial; font-size:11px; text-align:center'>Relat&oacute;rio Geral de Cadastros</td>
								</tr>";
				...
				for($k=1; $k<=$j; $k++){
					echo "	<div id='cadastros-container'>
								<div class='titulo-container'>
									<div class='titulo' style='min-height:25px'>
										<p style='margin-top:2px;'>
											Centro de Custo ".$centroCusto[$k]." | Respons&aacute;vel: ".$responsavel[$k]."
										</p>
									</div>
									<div class='conteudo-interno'>";
					geraTabela($largura,$colunas,$dados[$k], null, 'cadastro-localiza', 2, 2, 500,1);
					echo "		</div>
								</div>
							</div>";

					$html .= "<tr><td class='tabela-fundo-escuro-titulo' styledestaque align='left' colspan='10'><b>Centro de Custo ".$centroCusto[$k]."</b><br>Respons&aacute;vel: ".$responsavel[$k]."</td></tr>";
					$html .= "<tr><td align='left' stylenormal Style='margin:2px 5px 0 2px;' colspan='10'>".returnTabelaExcel($largura,$colunas,$dados[$k])."</td></tr>";
				}
			}
			else{
