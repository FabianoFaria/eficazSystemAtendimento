...
/********************************* Inicio Funções Respostas *********************************/
	...
	function salvarProcessoRepresentante(){
		global $dadosUserLogin;
		...
		if($tipo = mpress_fetch_array($tipos)) $situacao = $tipo[Descr_Tipo];
		...
		$sql = "insert into telemarketing_follows (Workflow_ID, Descricao, Dados, Situacao_ID, Motivo_ID, Responsabilidade_ID, Usuario_Cadastro_ID)
											values	('$workflowID', '$descricaoFollow', '$outros', '$situacaoID', '$motivoID', '', '$usuarioID')";
		mpress_query($sql);
		echo $workflowID;
	}
